...
function buildPrompts(o: PromptBuildOpts): { systemPrompt: string; userMessage: string } {
  ...
  const systemPrompt = [
    ...
    "      E.g. a researcher they cite is giving a one-night talk; their favourite obscure band visits.",
    "  (B) HYPERLOCAL MAGIC — once-a-year or rarer city-wide cultural moment (festes de barri, heritage open-days,",
    "      hyperlocal traditions, neighborhood-association anniversaries, parish festivals). The kind of event",
    ...
    "    State-blocking factors include weather (outdoor + heavy rain forecast), implicit constraints from the profile",
    "    (injury implied by recent context, a kid-week, travel). Friction lowers personal-fit but does NOT lower",
    "    hyperlocal-magic — a rain-soaked Cors Muts is still a Cors Muts.",
    "  - Reject conjecture on axis A: if the profile doesn't actually support 'this person would love it', don't pad.",
    "  - The 'why' must cite either the specific profile element (axis A) OR the specific rarity hook (axis B).",
    "    Generic 'matches your interests' or 'looks interesting' is forbidden — say WHICH interest / belief / trait,",
    "    or WHAT makes the event hyperlocally rare.",
    "  - Output ONLY a JSON object, no prose, no markdown fences:",
    '    {"scored":[{"event_id":string,"score":0-1,"why":"one sentence citing profile or rarity"}]}',
